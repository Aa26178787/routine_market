...
요구사항 명세서의 미확정 사항 중 ERD 작성을 위해 다음 권장안을 적용했다.
...
이 결정에 변경이 생기면 Django 모델과 첫 마이그레이션을 만들기 전에 본 문서를 먼저 수정한다.

## 9. 다음 설계 단계

1. 모델 필드와 제약조건 최종 검토
2. Django 앱 및 프로젝트 디렉터리 생성
3. 커스텀 `User` 모델을 포함한 초기 모델 구현
4. 첫 마이그레이션 생성
5. 관리자 등록 및 샘플 데이터 작성
6. 모델 제약조건과 권한에 대한 테스트 구현
